 function Pipeline_NetworkAnalysis
...
if(do_seed_nodes || do_module_search)
    ...
    if ( exist(networkFile, 'file') == 2)   % .mat file with the same name already exist
        yes_no = input('A network file for this input file has already been created. \nDo you want to overwrtie it? y/n : ','Import network') ;
        if (strcmp(yes_no,'n') == 1)
            do_it = 0;
        end
    end
    if (do_it)
    ...
    if ( exist(ExperimentalDataFile, 'file') == 2)   % .mat file with the same name already exist
        yes_no = input('A phenotypic file for this input file has already been created. \nDo you want to overwrtie it? y/n : ','Import profiles');
        if (strcmp(yes_no,'n') == 1)
            do_it = 0;
        end
    end
